...
plan = addTask(plan, "ci:validate:AHRS_Voter",     "Validate AHRS Voter model",       ["ci:init"]);
plan = addTask(plan, "ci:validate:Flight_Control", "Validate Flight Control model",    ["ci:init"]);
plan = addTask(plan, "ci:analyze:AHRS_Voter",      "Run analysis on AHRS Voter",      ["ci:validate:AHRS_Voter"]);
plan = addTask(plan, "ci:analyze:Flight_Control",  "Run analysis on Flight Control");
plan = addTask(plan, "ci:report",                  "Generate report from analysis",    ["ci:analyze:AHRS_Voter", "ci:analyze:Flight_Control", "ci:init"]);
